const details = () => ({
    id: 'Tdarr_Plugin_vdkb_Intel_Arc_HEVC_Transcode_Alt',
    Stage: 'Pre-processing',
    Name: 'Intel Arc QSV HEVC Transcode Alt',
    Type: 'Video',
    Operation: 'Transcode',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
	...
	//	response.infolog += `File is: ${file.fileMedium}\n`;
    //    return response;
    //}
    ...
    const pixel10Bit = inputs.force10bit ? '-vf "format=p010le,hwupload=extra_hw_frames=64" ' : '';
    const profile = inputs.force10bit ? 'main10' : 'main';

    // encoding settings
    response.preset += `-init_hw_device qsv=hw -filter_hw_device hw <io> -map 0 -c:v hevc_qsv -preset ${inputs.ffmpegPreset}`
